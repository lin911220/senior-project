...
import pyautogui  # 使用 pyautogui 來截圖

def send_screen_stream(host, port):
    screen_running = True  # 將 screen_running 設定為 True
...
def handle_client_commands(conn,client_ip):
    while True:
        message = conn.recv(1024).decode('utf-8')  # 接收來自客戶端的指令
        print(message)
        if  message == '0':
            break
        if message.startswith('1'):
            num_screenshots = int(message.split()[1])  # 從指令中提取截圖張數
            screenshot(conn, num_screenshots)
        elif message.startswith('2'):
            s_duration = int(message.split()[1])
            record_screen(duration=s_duration, client_socket=conn)  # 錄製並發送文件
        elif message.startswith('3'):
            c_duration = int(message.split()[1])
            ...
            video_thread = threading.Thread(target=send_video_stream, args=(client_ip, 5001))
            video_thread.start()
        elif message == 'G':
            screen_thread = threading.Thread(target=send_screen_stream, args=(client_ip, 5002))
            screen_thread.start()
        try:
...
def server_program():
    ...
    while True:
        ...
        # 啟動處理客戶端指令的線程
        client_thread = threading.Thread(target=handle_client_commands, args=(conn,client_ip))
        client_thread.start()
