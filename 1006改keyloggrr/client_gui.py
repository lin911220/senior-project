    ...
    if client_socket:
        ...
        try:
            if command == "1":
                ...
                print('停止鍵盤記錄')
                #client_socket.send("7".encode())  # 發送停止命令
...
# # 日誌顯示區域
log_text = Text(root, state='disabled', width=70, height=15, bg="#ffffff")
log_text.pack(padx=10, pady=10)

# # 視頻流區域
video_frame = ttk.LabelFrame(root, text="視頻流")
...
# 圓餅圖區域
pie_chart_frame = ttk.LabelFrame(root, text="類別圓餅圖")
pie_chart_frame.pack(padx=6, pady=6, fill="both", expand=True)

fig, ax = plt.subplots(figsize=(7, 4))  # Create a figure and axis for the pie chart
...
file_button1.pack(side="left", padx=5)  # Add padding between buttons

# Select analysis file button
file_button2 = Button(button_frame, text="選擇要分析CSV文件", command=select_and_process_file)
file_button2.pack(side="left", padx=5)  # Add padding between buttons
