...
# 設置樣式
style = ttk.Style()
style.configure("TLabelframe.Label", font=("Arial", 12))  # 設置字體為 Arial，大小為 14
...
# IP entry
ip_label = Label(connection_frame, text="Server IP:", bg="#f0f0f0", font=("Arial", 12))
ip_label.grid(row=0, column=0, padx=10, pady=5, sticky="w")
ip_entry = Entry(connection_frame, width=30, font=("Arial", 12))
ip_entry.grid(row=0, column=1, padx=5, pady=5)

# Port entry
port_label = Label(connection_frame, text="Server Port:", bg="#f0f0f0", font=("Arial", 12))
port_label.grid(row=1, column=0, padx=10, pady=5, sticky="w")
port_entry = Entry(connection_frame, width=30,font=("Arial", 12))
port_entry.grid(row=1, column=1, padx=5, pady=5)

# Connect button
connect_button = Button(connection_frame, text="Connect", command=connect_to_server, bg="#4CAF50", fg="white", width=14, height=1, font=("Arial", 12) )  # 設置寬度和高度
connect_button.grid(row=2, column=1, columnspan=1, pady=4) 

# Disconnect button
disconnect_button = Button(connection_frame, text="Disconnect", command=disconnect_from_server, bg="#F44336", fg="white", width=14, height=1, font=("Arial", 12) ) # 設置寬度和高度
...
# Command selection
ttk.Label(command_frame, font=("Arial", 12), text="選擇命令:").grid(column=0, row=0, padx=5, pady=5, sticky="w")
command_combo = ttk.Combobox(command_frame, state="readonly", width=40, font=("Arial", 12))  # 設置寬度和字體大小
...
# Send button
send_button = Button(command_frame, text="發送", command=send_command, bg="#2196F3", fg="white", width=14, height=1, font=("Arial", 12) ) # 設置字體大小  # 設置寬度和高度
...
# 日誌顯示區域
log_text = Text(log_frame, state='disabled', width=75, height=33, bg="#ffffff")
...
# 選擇文件按鈕
file_button1 = Button(button_frame, text="選擇CSV文件",command=select_file, width=15, height=1)  # 設置寬度和高度
file_button1.pack(side="left", padx=5)  # 以左右並排放置

# 選擇分析文件按鈕
file_button2 = Button(button_frame, text="選擇要分析CSV文件", command=select_and_process_file, width=15, height=1)  # 設置寬度和高度
